...
private:
	...
	void text_test(uint64_t max) {
	    ...
	    std::string test_text_join = join(test_text, "\n");
	    std::vector<std::vector<float>> input_vectors = embedding(test_text_join);
		...
		idx = 0;
		for (i = 0; i < max; ++i) {
			auto res = store.search_knn_hnsw_vector(input_vectors[i], k);
			for (auto j : res) {
				EXPECT(ans[idx], j.second);
				idx++;
			}
